...
// SVG hero image: 큰 이모지 + 작목명 + 카테고리 색상
function buildHeroSvg({ name, emoji, bg, category }) {
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 800" width="800" height="800">
  ...
  <circle cx="400" cy="370" r="200" fill="#ffffff" opacity="0.55"/>
  <text x="400" y="430" font-size="260" text-anchor="middle" dominant-baseline="middle" font-family="Apple Color Emoji, Segoe UI Emoji, sans-serif">${emoji}</text>
  <text x="400" y="640" font-size="56" font-weight="700" text-anchor="middle" font-family="-apple-system, 'Apple SD Gothic Neo', 'Noto Sans KR', sans-serif" fill="#222">${safe}</text>
  <text x="400" y="690" font-size="22" text-anchor="middle" font-family="-apple-system, 'Apple SD Gothic Neo', 'Noto Sans KR', sans-serif" fill="#888">${category} · 꼬꼬마텃밭</text>
</svg>`;
}

async function uploadHero(crop) {
  const svg = buildHeroSvg({ name: crop.name_ko, emoji: crop.emoji, bg: crop.bg, category: crop.category });
  const base64 = Buffer.from(svg, 'utf8').toString('base64');
  const r = await imagekit.upload({
    file: base64,
    fileName: `${crop.name_en.toLowerCase()}.svg`,
    folder: '/little-farm/crops',
