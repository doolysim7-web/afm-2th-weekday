...
## 일지 목록·수정·삭제 UX 개선 테스트 (커밋 `8323302`)

| # | 시나리오 | 결과 | 캡처 |
|---|---|---|---|
| 46 | 일지 목록 — 총 카운트 + 인라인 ✎ 수정 / ✕ 삭제 버튼 | ✅ 5개 카드, 각각 우측 버튼 | `46-logs-list-with-edit-delete.png` |
| 47 | 마이페이지 "📔 최근 일지" 섹션 (3건) + 쓰기/전체보기 | ✅ | `47-mypage-recent-logs.png` |
| 48 | 데스크톱 헤더에 "📔 내 일지" / "💰 가계부" 텍스트 라벨 (lg 이상) | ✅ | `48-desktop-header-labels.png` |
| 49 | ✕ 삭제 → 확인 다이얼로그 → 카드 사라짐 + 카운트 4로 갱신 | ✅ 5→4, 바질·옥수수 일지 제거 | `49-after-inline-delete.png` |
| 50 | ✎ 수정 클릭 → 수정 폼 진입, 기존 작목/제목/본문/기분/공개 모두 복원 | ✅ "3개 선택됨" + ✓ 칩 | `50-edit-form-restored.png` |
| 51 | 모바일(390x844) 하단 nav에 "📔 내 일지" 추가 (5개 항목) | ✅ | `51-mobile-nav-with-logs.png` |

### 핵심 검증

- **진입 다양화**: 헤더(데스크톱+모바일), 마이페이지 카드/최근 일지, "+ 일지" 버튼 — 어디서든 한 번에
- **인라인 수정/삭제**: 상세 페이지 안 거치고 목록에서 바로 처리. 삭제 확인 → 카드만 사라지고 페이지 유지
- **수정 폼 복원**: 다중 작목 ✓ 표시, 텍스트, 셀렉트 모두 정상 로드 (다중 작목 v1 호환)
- **모바일 우선**: 5번째 nav 항목으로 일지가 캘린더·작목 사이에 배치, 회원에게만 노출
