...
> 작성일: 2026-05-10 *(week-6 초안 2026-04-26을 갱신)*
> 기간: **2주(14일) + 데모데이** — 2026-05-11(월) ~ 2026-05-24(일)
> 1인 개발 + 일평균 4~6시간 작업 가정
> 컨셉: **주말농장러를 위한 작고 따뜻한 텃밭 동반자**

---

## 0. 선행 검증 (week-7 진행 노트)

> 2주 전 초안 작성 이후, 동일 스택(Express 5 + pg + Supabase + JWT + ImageKit + Vercel)을 [당근마켓 클론](../당근마켓%20클론/)에서 end-to-end로 검증함. 아래 패턴은 그대로 차용 가능 → 위험 항목 일부 하향.

| 항목 | 검증 내용 | DEV 영향 |
|---|---|---|
| Express 5 + Vercel `@vercel/node` 배포 | `vercel.json` rewrites + serverless export 정상 | D1·D14 일정 단축 |
| Supabase Postgres + 테이블 prefix + lazy `CREATE IF NOT EXISTS` | 첫 요청 시 13개 테이블 자동 생성 가능 | D1 부담 낮음 |
| JWT 7일 + bcryptjs + `authRequired`·`adminOnly` 미들웨어 | 가입/로그인/권한 분리 확인 | D2 1일 안에 가능 |
| ImageKit 클라이언트 직접 업로드 (HMAC-SHA1 서명) | 서버는 서명만 발급, 파일은 클라이언트→ImageKit 직송 | D5 일지 사진 업로드 그대로 적용 |
| 폴링 기반 실시간 (`?since=lastId`, 2초 주기) | 채팅 메시지 폴링 검증 | D8 시기 알림 폴링도 동일 패턴 사용 |
| 낙관적 UI 업데이트 + AbortController로 in-flight fetch 취소 | 빠른 페이지 전환 시 콘솔 노이즈 0 | UX 폴리시 (D12)에 반영 |
| 관리자 콘솔 (is_admin 컬럼 + 통계/사용자/콘텐츠 CRUD) | 별도 라우트 `#/admin` 구성 검증 | D11 단축 |
...
- **`pg` 풀 + Lazy 스키마 init** (당근마켓 클론에서 검증된 `CREATE IF NOT EXISTS` 패턴)
...
### 배포
- **Vercel** (`@vercel/node` + `@vercel/static`, vercel.json은 당근마켓 클론 패턴 그대로)
...
## 6. 14일 WBS (2026-05-11 ~ 05-24, 데모데이 포함)

> 색깔 표기: **DB · 백엔드 · 프론트 · AI/결제 · 폴리시·발표**

### 1주차 — 토대 만들기

**D1 (월) 5/11 — 프로젝트 셋업 + 스키마 확정**
- `app/` 폴더 + Vercel 프로젝트 링크 (당근마켓 클론 vercel.json 답습)
...
**D5 (금) 5/15 — 농사일기 (백 + 프론트)**
- `POST/GET/PUT/DELETE /api/logs`
- ImageKit 서명 엔드포인트 + 클라이언트 직접 업로드 (당근마켓 클론과 동일 흐름)
- 일지 캘린더 뷰 + 카드 리스트
- 공개범위 토글: 비공개/친구공개/전체공개
- 친구공개: 카카오 공유 링크로 들어온 사람만 열람

**D6 (토) 5/16 — 농사 가계부 (백 + 프론트)**
...
- 친절체 마이크로카피
- 폴링 기반 실시간 갱신 (당근마켓 클론 채팅 폴링과 동일 패턴)

**D9 (화) 5/19 — AI 보강 (Gemini)**
...
**D11 (목) 5/21 — 카카오 공유 + 관리자 콘솔**
- 카카오 SDK로 친구 공유 (메인·작목 가이드·내 일지[공개])
- 공유 시 share_token 발급 → 클릭/가입 추적
- `#/admin`: 작목 CRUD, 게시글 숨김, 신고 처리, 매출·가입자 카드 (당근마켓 클론 admin 패턴 답습)

**D12 (금) 5/22 — 모바일 PWA + 접근성**
- `manifest.json`, 앱 아이콘 (꼬꼬마텃밭 로고: 작은 새싹+해)
- 홈 화면 추가 안내 배너
- "큰 글씨 모드" 토글 (어르신 사용자용)
- 모바일 사파리/크롬 실기기 테스트
- 라이트하우스 90+ 목표
- 폴리시: 낙관적 UI 업데이트(좋아요·관심)·AbortController로 in-flight fetch 정리

**D13 (토) 5/23 — 통합 테스트 + 시드 + 데모 시나리오**
...
| 위험 | 영향 | 완화 | 변화(week-7) |
|---|---|---|---|
| 작목 콘텐츠 작성에 시간 폭주 | 일정 지연 | D1에 12개 작목 우선, Gemini로 초안 → 사람 검수 | 동일 |
| Gemini 이미지 품질 들쭉날쭉 | UX 저하 | 1차는 Unsplash 스톡 + 농진청 자료, AI는 보조 | 동일 |
| Toss 결제 환불·CS | 시간 소모 | 단가 5만원 이하만, 7일 무조건 환불 | 동일 |
| 알림 노이즈로 사용자 피로 | 이탈 | 빈도 주 1회 캡, "받지 않기" 토글 | 동일 |
| 카카오 공유 어뷰징 | 스팸 | share_token 하루 한도 + user 묶기 | 동일 |
| 농사 정보 오류 → 작물 죽음 | 평판 | 가이드 페이지에 "참고용 / 지역 보정 권고" 명시 | 동일 |
| 1인 개발 일정 지연 | 모든 것 | D5/D7/D11 강제 회고 → 비핵심 v1.1로 미룸 | **선행 검증 덕에 D1·D2·D11 부담 ↓** |
| Express 5 + Vercel 배포 트러블 | 출시 지연 | — | **검증됨 → 위험 없음** |
| ImageKit 직접 업로드 흐름 | 사진 누락 | — | **검증됨 → 위험 없음** |
...
- **2026-04-26**: 초안 작성 (week-6) — D1 = 4/27 가정
- **2026-05-10**: WBS를 5/11~5/24로 재편, 0절 "선행 검증" 추가, 위험 표에 검증 결과 반영. 기능·스키마·API 동일

---

## 한 페이지 결론

> 단일 `index.html` + `server.js` + Postgres(`little_farm_*`)로 14일에 9개 핵심 기능을 굴린다. 비로그인 캘린더가 진입을 잡고, 일지·가계부·결제로 묶어두고, 카카오 공유로 더 끌어온다. **꼬꼬마텃밭** 톤은 "어르신 서비스 같지 않으면서도 친절한 말투" — 마이크로카피와 색채로 차별화한다. 동일 스택을 당근마켓 클론으로 이미 굴려본 만큼, 이번 14일은 **콘텐츠(작목·작업 시드)와 텃밭 도메인 디자인**에 시간을 더 쏟는다.
